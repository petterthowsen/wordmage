...
module WordMage
  ...
  class Generator
    ...
    private def generate_with_complexity_budget(syllable_count : Int32, starting_type : Symbol?, initial_budget : Int32, original_syllable_count : Int32 = syllable_count) : String
      ...
      # Apply gemination if enabled (global or syllable-level)
      if @gemination_probability > 0.0 || syllables.any? { |s| 
        template = get_template_for_syllable(s)
        template && template.gemination_probability > 0.0
      }
        phonemes = apply_gemination_with_templates(phonemes, syllables)
      end

      # Apply vowel lengthening if enabled (global or syllable-level)
      if @vowel_lengthening_probability > 0.0 || syllables.any? { |s| 
        template = get_template_for_syllable(s)
        template && template.vowel_lengthening_probability > 0.0
      }
        phonemes = apply_vowel_lengthening_with_templates(phonemes, syllables)
      end
    ...
    # Gets the template used for a specific syllable (simplified approach)
    private def get_template_for_syllable(syllable : Array(String)) : SyllableTemplate?
      # Find a template that could have generated this syllable
      @word_spec.syllable_templates.find { |t| syllable_matches_template?(syllable, t) }
    end

    # Checks if a syllable could have been generated by a template
    private def syllable_matches_template?(syllable : Array(String), template : SyllableTemplate) : Bool
      # Simplified matching - check if pattern length and structure could match
      pattern = template.pattern
      return false if syllable.empty?
      
      # Count consonants and vowels in syllable
      consonant_count = syllable.count { |p| !@phoneme_set.is_vowel?(p) }
      vowel_count = syllable.count { |p| @phoneme_set.is_vowel?(p) }
      
      # Count expected consonants and vowels in pattern
      expected_c = pattern.count('C')
      expected_v = pattern.count('V')
      
      # Allow for hiatus (VV) and gemination in matching
      consonant_count >= expected_c && vowel_count >= expected_v
    end

    # Apply gemination with template-specific probabilities
    private def apply_gemination_with_templates(phonemes : Array(String), syllables : Array(Array(String))) : Array(String)
      return phonemes if phonemes.size < 2
      ...
      while i < phonemes.size
        ...
        current_syllable = syllables[syllable_index]? if syllable_index < syllables.size
        
        # Only geminate consonants that are in valid positions for gemination
        if i < phonemes.size - 1 && 
           can_geminate_at_position?(phonemes, i)
          
          # Get template-specific probability if available
          template = current_syllable ? get_template_for_syllable(current_syllable) : nil
          template_prob = template ? template.gemination_probability : 1.0_f32
          
          # Multiply global probability by template probability
          effective_prob = @gemination_probability * template_prob
          
          # Calculate enhanced probability for this specific consonant
          gemination_prob = calculate_gemination_probability_with_base(current_phoneme, effective_prob)
        ...
        # Update syllable tracking
        syllable_position += 1
        if current_syllable && syllable_position >= current_syllable.size
          syllable_index += 1
          syllable_position = 0
        end
    ...
    # Apply vowel lengthening with template-specific probabilities
    private def apply_vowel_lengthening_with_templates(phonemes : Array(String), syllables : Array(Array(String))) : Array(String)
      return phonemes if phonemes.size < 1
      ...
      while i < phonemes.size
        ...
        if i < phonemes.size - 1 && 
          ...
          # Get template-specific probability if available
          template = current_syllable ? get_template_for_syllable(current_syllable) : nil
          template_prob = template ? template.vowel_lengthening_probability : 1.0_f32
          
          # Multiply global probability by template probability
          effective_prob = @vowel_lengthening_probability * template_prob
          
          # Calculate enhanced probability for this specific vowel
          lengthening_prob = calculate_vowel_lengthening_probability_with_base(current_phoneme, effective_prob)
    ...
    # Calculate gemination probability with custom base probability
    private def calculate_gemination_probability_with_base(consonant : String, base_prob : Float32) : Float32
      # If we have analysis patterns, use them to boost specific consonants
      if !@gemination_patterns.empty?
        # Look for patterns like "gg", "ll", "nn", etc.
        gemination_pattern = consonant + consonant
    ...
    # Calculate vowel lengthening probability with custom base probability
    private def calculate_vowel_lengthening_probability_with_base(vowel : String, base_prob : Float32) : Float32
      # If we have analysis patterns, use them to boost specific vowels
      if !@vowel_lengthening_patterns.empty?
        # Look for patterns like "aa", "ee", "oo", etc.
        lengthening_pattern = vowel + vowel
